...
import { labelFromFinalScore, topRiasecToVector } from "./types.js";
...
export type ExportFilters = {
  ...
  bacStream?: string;
  specialtyCode?: string;
  /** When true (default for analytics export), omit student_name column. */
  anonymized?: boolean;
};
...
export const exportEvaluationsAsCsv = (filters: ExportFilters = {}): string => {
  const anonymized = filters.anonymized !== false;
...
/** B0 analytics filters (same axes as export). */
...
export type AnonymizedEvaluationRow = {
  sessionRef: string;
  evaluatedAt: string;
  bacStream: string;
...
/**
 * One evaluation session = one student row.
 * Top specialty = rank_position = 1.
 * Match label is derived from final_score (not stored).
 */
export const getAnalyticsSummary = (filters: AnalyticsFilters = {}): AnalyticsSummary => {
  const { where, params } = buildEvaluationWhere(filters);

  // Restrict to rank 1 so each session counts once for stream / specialty / label.
...
export const getRecentEvaluationsAnonymized = (
  filters: AnalyticsFilters = {},
  limit = 50,
): AnonymizedEvaluationRow[] => {
  const { where, params } = buildEvaluationWhere(filters);
  const rankClause = where ? `${where} AND me.rank_position = 1` : `WHERE me.rank_position = 1`;
  ...
  return rows.map((row) => ({
    sessionRef: row.student_id.slice(0, 8),
