    ...
    <div className="min-w-0 space-y-6">
      ...
      <div className="grid min-w-0 gap-4 sm:grid-cols-2">
        ...
        <section className="analytics-card min-w-0 p-4">
          ...
          {(profile.topRiasec ?? profile.riasec?.topRiasec)?.length ? (
            <ul className="mt-3 space-y-1.5 text-sm">
              {(profile.topRiasec ?? profile.riasec?.topRiasec ?? []).map((entry, i) => (
      ...
      <section className="min-w-0">
        ...
        <div className="space-y-0">
          {(profile.matches ?? []).map((match) => (
            <article
              key={match.specialtyId ?? match.specialtyCode ?? String(match.rank)}
              ...
              <div className="flex min-w-0 flex-wrap items-start justify-between gap-2">
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-0.5 break-words font-mono text-xs text-ink-muted">
                    ...
                    {(match.hollandCode?.length ?? 0) > 0 ? ` · ${match.hollandCode!.join("-")}` : ""}
                  </p>
                </div>
                <div className="shrink-0 text-right">
                  <div className="font-mono text-lg font-medium tabular-nums text-ink">
                    {Number(match.finalScore).toFixed(1)}%
                  </div>
                </div>
              </div>
            </article>
