...
export function Step1AcademicForm({
  ...
  return (
    <div className={`space-y-8 text-left font-body ${disabled ? "pointer-events-none opacity-60" : ""}`}>
      ...
      <hr className="intended-rule" />

      {/* Stream */}
      <section className="space-y-4">
        <p className="font-mono text-[10px] font-medium uppercase tracking-[0.14em] text-ink-muted">
          {lang === "fr" ? "Filière" : "Stream"}
        </p>
        <label className="block">
          ...
          <section className="space-y-4">
            <div>
              ...
              <p className="mt-1 text-xs text-ink-muted">
                {lang === "fr"
                  ? "Notes des matières utilisées pour le score académique (/20)."
                  : "Subject marks used for the academic score (/20)."}
              </p>
            </div>
              ...
              {slotOrder.map((slotKey) => {
                ...
                return (
                  <label key={slotKey} className="block">
                    <span className="text-[10px] font-semibold uppercase tracking-wide text-brass">
                      {slotLabel[slotKey]}
                    </span>
